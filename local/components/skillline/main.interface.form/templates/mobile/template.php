...
if (!$arParams["TIME_FORMAT"])
    $arParams["TIME_FORMAT"] = preg_replace(['/[dDjlFmMnYyo]/', '/^[\/.,\s]+/', '/[\/.,\s]+$/'], "", $arParams["DATE_TIME_FORMAT"]);
...
CUtil::InitJSCore(['ajax', 'date', "mobile_fastclick", "uploader"]);
...
<div class="mobile-grid mobile-grid-entity <? if ($arParams["RESTRICTED_MODE"]) echo "mobile-grid-restricted"; ?> ">
    ...
    <form name="<?=$arParams["FORM_ID"]?>" id="<?=$arParams["FORM_ID"]?>" action="<?=POST_FORM_ACTION_URI?>"
          method="POST" enctype="multipart/form-data" class="business-process__form--add">
        <?=bitrix_sessid_post();?>
        <input type="hidden" id="<?=$arParams["FORM_ID"]?>_active_tab" name="<?=$arParams["FORM_ID"]?>_active_tab"
        ...
        <div class="bx-tabs"><?
            ...
            <div id="tab_<?=$tab["id"]?>">
                <div
                    ...
                    <? if ($tab["icon"] <> ""): ?>
                        <div class="bx-icon <?=htmlspecialcharsbx($tab["icon"])?>"></div> <? endif; ?>
                    <?/*?><div class="bx-form-title"><?=htmlspecialcharsbx($tab["title"])?></div><?*/ // Our design is not support this 13.11.2015
                    ?>
                    <div style="height: 100%;">
                        <div class="bx-edit-table <?=(isset($tab["class"]) ? $tab['class'] : '')?>"
                             id="<?=$tab["id"]?>_edit_table"><?
                            /**
                             * File
                            ...
                            <div class="mobile-grid-field"
                                 id="<?=$field["id"]?>" <? if (!empty($style)): ?> style="<?=$style?>"<? endif ?>><?
                                if (array_key_exists("name", $field)) {
                                    ?>
                                    <div class="mobile-grid-title"><?=htmlspecialcharsEx($field["name"])?></div><?
                                }
                                ?><input id="checkbox_<?=$field["id"]?>"
                                         type="checkbox" <?=($expanded ? "checked" : "")?> value="section"/><?
                                ?><label for="checkbox_<?=$field["id"]?>"
                                         class="mobile-grid-field-switcher"><?=htmlspecialcharsbx($field["value"])?></label><?
                                ?>
                                <div class="mobile-grid-body"
                                     id="section_<?=$field["id"]?>_body"<? if (!empty($style)): ?> style="<?=$style?>"<? endif ?>><?
                                    }
                                    else
                                    {
                                    ?>
                            ...
                            switch ($field["type"]) {
                                ...
                                case 'textarea':
                                    ...
                                    if ($arParams["RESTRICTED_MODE"]) {
                                        ...
                                        $html = "<textarea class=\"mobile-grid-data-textarea\" name=\"{$field["id"]}\" id=\"{$field["~id"]}\" $params>$val</textarea>";
                                    }
                                    $jsObjects[] = $field["~id"];
                                    break;
                                case 'select-group':
                                    ...
                                    if ($field["items"]) {
                                        ...
                                        foreach ($field["items"] as $item) {
                                            ...
                                            elseif (is_array($field["menu"])):
                                                ?><i class="mobile-grid-menu"
                                                     id="<?=$field["~id"]?>_menu_<?=$item["id"]?>"></i><?
                                            endif;
                                            ?>
                                            <div class="avatar"<? if (!empty($item["avatar"])): ?> style="background-image:url('<?=htmlspecialcharsbx($item["avatar"])?>')"<? endif; ?>></div>
                                            <?php if ($field['type'] === 'select-user'): ?>
                                ...
                                case 'select':
                                    ...
                                    if (is_array($field["items"])) {
                                        ...
                                        if (empty($selected) && !(is_array($field["params"]) && array_key_exists("multiple", $field["params"]))) {
                                            $selected = [reset(array_keys($field["items"]))];
                                        }
                                ...
                                case 'checkbox':
                                    $className = "checkbox";
                                    $items = (is_array($field["items"]) ? $field["items"] : ["Y" => "Y"]);
                                    $val = (is_array($val) ? $val : [$val]);
                                ...
                                case 'image':
                                    ...
                                    <div class="mobile-grid-button file" id="file-eventnode-<?=$field["~id"]?>"><?
                                        ...
                                        ?><input class="mobile-grid-button-file" type="file"
                                                 id="<?=$field["~id"]?>_file" size="1" <? if ($field["maxCount"] != 1){
                                                 ?>multiple="multiple" <?
                                        } ?>/></div>
                                    <?
                                    $html = ob_get_clean();
                            ...
                            if (array_key_exists("name", $field)) {
                                ?>
                                <div
                                class="mobile-grid-title <?=($field["required"] ? " mobile-grid-title-required" : "")?>" <?
                            ...
                            ?>
                            <div
                            class="mobile-grid-block mobile-grid-data-<?=$className?>-container"><?=preg_replace("/[\t\n]+/i", "", $html)?></div><?
                            ?></div><?
                            }
                            }
                            ?>
...
<? endif; ?>
    <script>
        ...
        BX.ready(function () {
            new BX.Mobile.Grid.Form(<?=CUtil::PhpToJSObject([
                ...
                "format" => (isset($arParams["~DATE_TIME_FORMAT"]) ? ["datetime" => $arParams["~DATE_TIME_FORMAT"]] : []) +
                    (isset($arParams["~DATE_FORMAT"]) ? ["date" => $arParams["~DATE_FORMAT"]] : []) +
                    (isset($arParams["~TIME_FORMAT"]) ? ["time" => $arParams["~TIME_FORMAT"]] : []),
